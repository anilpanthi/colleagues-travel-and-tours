...
export const PackageDetails: React.FC<PackageDetailsProps> = ({ pkg }) => {
	...
	const hasHeroImage = Boolean(pkg?.hero?.type && pkg?.hero?.type !== 'none' && pkg?.hero?.type !== 'lowImpact')
					...
					<div className={style.singlePackage__left}>
						...
						{pkg.overview && (
							<div className={style.overviewSection}>
								<h2 className={style.title}>{pkg.title}</h2>
								<ReadMore lines={10}>
						...
						{(pkg.includes || pkg.excludes) && (
							<div className={style.includeExcludeSection}>
								...
								<div className={style.includeExclude_contents}>
									{pkg.includes && (
										<div className={style.includecontent}>
											<RichText
												className={style.includesList}
												data={pkg.includes}
												enableGutter={false}
											/>
										</div>
									...
									{pkg.excludes && (
										<div className={style.excludecontent}>
											<RichText
												className={style.excludesList}
												data={pkg.excludes}
												enableGutter={false}
											/>
										</div>
